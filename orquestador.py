...
TIPO_LEGADO_POR_CATALOGO = {
    ...
    # Repuestos XECURO reusa portada/contraportada/fondo del catálogo general
    # (misma marca, mismo set de assets XECURO).
    'xecuro_repuestos':     'cascos',
    'xtrong_integrales':    'integrales',
    'xtrong_otros_cascos':  'abatibles_abiertos',
    'xtrong_accesorios':    'textiles_accesorios',
    # Repuestos XTRONG: contraportada y fondo de página se reusan de
    # "Textiles y accesorios"; la portada es propia (ver PORTADA_OVERRIDE
    # más abajo).
    'xtrong_repuestos':     'textiles_accesorios',
}

# Portadas propias por catálogo, que reemplazan la portada del "tipo legado"
# de TIPO_LEGADO_POR_CATALOGO (la contraportada y el fondo de página se
# siguen tomando del tipo legado). Archivo esperado en assets/{marca}/.
PORTADA_OVERRIDE_POR_CATALOGO = {
    'xtrong_repuestos': 'portada_repuestos.jpg',
}
...
def generar_catalogo(
    ...
    ruta_manifest_paginas = os.path.join(base_dir, 'data', 'paginas_manifest.json')
    indice_sku = paginas_sku.construir_indice_paginas(
        carpeta_paginas_sku, ruta_manifest=ruta_manifest_paginas, callback_log=log)
    ...
    nombre_portada = PORTADA_OVERRIDE_POR_CATALOGO.get(
        catalogo_id, assets_tipo.get('portada', 'PORTADA.jpg'))
    portada_path = gw._buscar_asset(carpeta_assets, nombre_portada)
