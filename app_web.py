...
def catalogos():
    """Lista los catálogos disponibles (botones del front), indicando además
    si cada uno tiene al menos un producto con inventario > 0 en el Excel
    (campo 'disponible'). El frontend usa ese flag para deshabilitar, por
    ejemplo, el botón de Repuestos cuando la marca no tiene inventario."""
    disponibilidad = {cid: False for cid in CATALOGOS}
    try:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        datos = cargar_grupos_activos(base_dir)
        for g in datos['grupos'].values():
            cid = g.get('catalogo')
            if cid in disponibilidad:
                disponibilidad[cid] = True
    except Exception:
        # Si no se puede leer el Excel acá, no bloqueamos el front: se asume
        # disponible y el propio flujo de generación reportará el error real.
        disponibilidad = {cid: True for cid in CATALOGOS}
        ...
        'catalogos': [
            {
                ...
                'marca':      cfg['marca'],
                'nombre':     cfg['nombre'],
                'disponible': disponibilidad.get(cid, True),
            }
