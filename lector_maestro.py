...
def cargar_grupos_activos(base_dir, ruta_excel=None, callback_log=None):
    ...
    for fila in filas[1:]:
        ...
        catalogo_id = catalogo_asignado(marca, cat_catalogo, subcategoria)
...
def _nombre_para_ordenar(grupo):
    """Nombre 'limpio' usado como criterio de desempate dentro de una referencia."""
    import re
    nombre = grupo.get('nombre_producto') or ''
    return re.sub(r'\s+', ' ', nombre).strip().upper()


import re as _re
# Código de referencia/modelo dentro del Nombre_catalogo: XTR-DREXO, XTR-M69,
# XTR-352R1, XTR-K-02, XR-217, Y10, Y10-2X, C-1413, etc. Se usa para que todas
# las páginas de la misma referencia queden en páginas contiguas.
_REFERENCIA_RE = _re.compile(
    r'\b(XTR-[A-Z0-9]+(?:-[A-Z0-9]+)?'
    r'|XR-[A-Z0-9]+(?:-[A-Z0-9]+)?'
    r'|Y\d+(?:-\d+X)?'
    r'|[A-Z]{1,3}-?\d+[A-Z0-9]*)\b',
    _re.I)


def _referencia_para_agrupar(grupo):
    """Devuelve la referencia (modelo) del grupo, en MAYÚSCULAS y sin espacios,
    para agrupar páginas contiguas: todos los XTR-DREXO juntos, todos los M69
    juntos, etc. Si el nombre no trae un código de modelo reconocible (p.ej.
    'Poncho', 'Covertor', 'Intercomunicador'), se usa el nombre completo para
    que los productos con el mismo nombre igual queden juntos."""
    nombre = grupo.get('nombre_producto') or ''
    m = _REFERENCIA_RE.search(nombre)
    if m:
...
def armar_catalogo(catalogo_id, grupos_activos, callback_log=None):
    ...
    - Agrupa por subcategoría (columna J) y ordena las secciones según
      categorias_config.orden_subcategoria(); dentro de cada sección, agrupa por
      referencia (todas las páginas del mismo modelo quedan contiguas) y usa el
      nombre como desempate.
    """
    def log(m):
        if callback_log:
            callback_log(m)
    ...
    for subcat in sorted(secciones.keys(), key=orden_subcategoria):
        # Agrupa por referencia (modelo) para que todas las páginas de la misma
        # referencia queden contiguas; el nombre solo desempata dentro de ella.
        grupos = sorted(
            secciones[subcat],
            key=lambda g: (_referencia_para_agrupar(g), _nombre_para_ordenar(g)),
        )
